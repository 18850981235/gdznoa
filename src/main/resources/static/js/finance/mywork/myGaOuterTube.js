...
function w() {
    ...
    $.ajax({
        ...
        success:function (data) {
            ...
            html+='<td colspan="4" style="text-align: center"><b>外经证</b></td>';
            html+='</tr>';
            ...
            html += '<input style="display:none" value='+data.list.id+'>';
            if(data.list.processNode==1){
                html += '<input style="display:none" value="账务部确认">';
            }else if(data.list.processNode==2){
                html += '<input style="display:none" value="账务部确认">';
            }
