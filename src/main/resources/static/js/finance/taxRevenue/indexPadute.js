...
function w() {
    ...
    $.ajax({
        ...
        success:function (date) {
            ...
            $("#center input").eq(2).val(data.type);
            ...
            $("#center input").eq(14).val(data.accessory);
            $("#center input").eq(15).val(data.id);
            $("#center input").eq(16).val(data.inorout);

            $(".type option").eq(0).val(data.companyName)
            $(".type option").eq(0).text(data.companyName)
