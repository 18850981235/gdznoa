...
function w() {
    var  id=q("id");
    var url="/ga/comego/update.json?id="+id;
    $.ajax({
        type:"get",
        dataType:"json",
        url:url,
        ...
        success:function (date) {
            ...
            $("#center input").eq(8).val(data.type2);

            $("#center input").eq(9).val(data.digest);
            $("#center input").eq(10).val(data.bankname);
            $("#center input").eq(11).val(data.bankaccount);
