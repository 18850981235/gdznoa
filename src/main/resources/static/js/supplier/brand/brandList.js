...
function lo(z) {
    ...
    $.ajax({
        type: "get",
        url: "/supplier/trademark/QueryTrademark",
        ...
        success: function (data) {
            ...
            $.each(list, function (index1, obj1) {
              ...
              html += '<tr class="seal" style="height: 40px; " >';
                  html += '<td style="width: 60px ">' + (index1+1) + '</td>';
                  html += '<td style="width: 250px ">'+obj1.enterpriseName +'</td>';
                  html += '<td style="width: 100px "> '+obj1.product +'</td>';
                  html += '<td style="width: 100px ">'+obj1.name +'</td>';
                  html += '<td style="width: 150px ">'+obj1.englishName +'</td>';
                  html += '<td style="width: 120px ">'+obj1.enterpriseName +'</td>';
                  html += '<td style="width: 100px ">'+obj1.originPlace +'</td>';
                  html += '<td style="width: 240px ">'+obj1.url +'</td>';
                html += '<td style="width:110px">';
                html += '<a style="color:blue" href="/supplier/trademark/queryTrademarkpage?id='+obj1.id+'">详情</a>';
                html += '<a style="color:#dea97d; margin-left: 4px " href="/supplier/trademark/updateTrademarkpage?id='+obj1.id+'">编辑</a>';
                html += '<a class='+"del"+index1+' style="color:#a6d1c0; margin-left: 4px">删除</a>';
                html += '<div class='+"confirm"+index1+' style="display: none" >';
                html += '<p style="margin-bottom:80px ">' + "是否删除客户联系人信息" + '</p>';

                html += '<a class="confirm-ok" type="submit" href="/supplier/trademark/delete" >' + "是" + '</a>';
                html += '<a class="confirm-no" type="submit" >' + "否" + '</a>';
                html += '</div>';
                html += '</td>';
                  html += '</tr>';
